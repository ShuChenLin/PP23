...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    __shared__ unsigned char s_shared[5][260][3];
    ...
    for (int y = 0; y < height; ++y) {
        ...
        if (y == 0) {
            for (int j = 0; j < 3; ++j) {
                s_shared[1][threadIdx.x + 2][j] = 0;
                if (threadIdx.x < 2 && x >= 2) {
                    s_shared[1][threadIdx.x][j] = 0;
                } else if (threadIdx.x >= 256 && x < width - 2) {
                    s_shared[1][threadIdx.x + 4][j] = 0;
                }
            }
            for (int i = 2; i < 5; ++i) {
                for (int j = 0; j < 3; ++j) {
                    s_shared[i][threadIdx.x + 2][j] = s[channels * (width * i + x) + j];
                }
            }
            if (threadIdx.x < 2 && x >= 2) {
                for (int i = 2; i < 5; ++i) {
                    for (int j = 0; j < 3; ++j) {
                        s_shared[i][threadIdx.x][j] = s[channels * (width * i + x - 2) + j];
                    }
                }
            } else if (threadIdx.x >= 256 && x < width - 2) {
                for (int i = 2; i < 5; ++i) {
                    for (int j = 0; j < 3; ++j) {
                        s_shared[i][threadIdx.x + 4][j] = s[channels * (width * i + x + 2) + j];
                    }
                }
            }
        } else {
            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 3; ++j) {
                    s_shared[i][threadIdx.x + 2][j] = s_shared[i + 1][threadIdx.x + 2][j];
                }
            }
            if (threadIdx.x < 2 && x >= 2) {
                for (int i = 0; i < 4; ++i) {
                    for (int j = 0; j < 3; ++j) {
                        s_shared[i][threadIdx.x][j] = s_shared[i + 1][threadIdx.x][j];
                    }
                }
            } else if (threadIdx.x >= 256 && x < width - 2) {
                for (int i = 0; i < 4; ++i) {
                    for (int j = 0; j < 3; ++j) {
                        s_shared[i][threadIdx.x + 4][j] = s_shared[i + 1][threadIdx.x + 4][j];
                    }
                }
            }
            if (y + 2 < height) {
                for (int j = 0; j < 3; ++j) {
                    s_shared[4][threadIdx.x + 2][j] = s[channels * (width * (y + 2) + x) + j];
                }
                if (threadIdx.x < 2 && x >= 2) {
                    for (int j = 0; j < 3; ++j) {
                        s_shared[4][threadIdx.x][j] = s[channels * (width * (y + 2) + x - 2) + j];
                    }
                } else if (threadIdx.x >= 256 && x < width - 2) {
                    for (int j = 0; j < 3; ++j) {
                        s_shared[4][threadIdx.x + 4][j] = s[channels * (width * (y + 2) + x + 2) + j];
                    }
                }
            }
        }
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                for (int u = -xBound; u <= xBound; ++u) {
                    if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                        const unsigned char R = s_shared[v + yBound][u + xBound + threadIdx.x][2];
                        const unsigned char G = s_shared[v + yBound][u + xBound + threadIdx.x][1];
                        const unsigned char B = s_shared[v + yBound][u + xBound + threadIdx.x][0];
...
int main(int argc, char **argv) {
    ...
    const int num_blocks = (width + num_threads - 1) / num_threads;
