...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    //for (y = 0; y < height; ++y) {
        //for (x = 0; x < width; ++x) {
            //for (i = 0; i < MASK_N; ++i) {
    for (y = 0; y < height; ++y) {
        for (x = tid; x < width; x += numThreads) {
            for (i = 0; i < MASK_N; ++i) {
                adjustX = (MASK_X & 1) ? 1 : 0;
                adjustY = (MASK_Y & 1) ? 1 : 0;
                xBound = MASK_X /2;
                yBound = MASK_Y /2;
...
int main(int argc, char** argv) {
    ...
    sobel<<<50, 100>>>(device_s, device_t, height, width, channels);
